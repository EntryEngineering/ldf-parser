...
export function parseString(ldfString: string): any {
    ...
    while (lines[actIndex] !== "}") {
        ...
        while (lines[actIndex].trim() !== "}") {
            ...
            if (attributeLine.startsWith("LIN_protocol")) attributes.LIN_protocol = attributeLine.split(/ |;/)[2];
            if (attributeLine.startsWith("configured_NAD")) attributes.configuredNAD = attributeLine.split("=")[1];
            if (attributeLine.startsWith("initial_NAD")) attributes.initialNAD = attributeLine.split("=")[1];
            if (attributeLine.startsWith("product_id")) attributes.productId = attributeLine.split(/=|;/)[1].split(", ");
            if (attributeLine.startsWith("response_error")) attributes.responseError = attributeLine.split("=")[1];
            if (attributeLine.startsWith("P2_min")) attributes.P2_min = parseFloat(attributeLine.split(' ')[2]);
            if (attributeLine.startsWith("ST_min")) attributes.ST_min = parseFloat(attributeLine.split(' ')[2]);
            if (attributeLine.startsWith("N_As_timeout")) attributes.N_As_timeout = parseFloat(attributeLine.split(' ')[2]);
            if (attributeLine.startsWith("N_Cr_timeout")) attributes.N_Cr_timeout = parseFloat(attributeLine.split(' ')[2]);
    ...
    while (lines[actIndex] !== "}") {
        const signalEncodingType: SignalEncodingType = {
            name: lines[actIndex].split(' ')[0].trim()
        }
        actIndex++;
        while (lines[actIndex].trim() !== "}") {
            ...
            if (valueLine[0] === "logical_value") {
                const logivalValue: LogicalValue = {
                    signalValue: parseInt(valueLine[1]),
                    textInfo: valueLine[2] ? valueLine[2].slice(1, -1) : undefined
                }
                if (signalEncodingType.logicalValues) signalEncodingType.logicalValues.push(logivalValue)
                else signalEncodingType.logicalValues = [logivalValue]
            } else if (valueLine[0] === "physical_value") {
                const physicalRange: PhysicalRange = {
                ...
                if (signalEncodingType.physicalRanges) signalEncodingType.physicalRanges.push(physicalRange)
                else signalEncodingType.physicalRanges = [physicalRange]
            }
            actIndex++;
        }
    ...
    while (lines[actIndex] !== "}") {
        ...
        if (result.signalEncodingTypes[line[0]]) result.signalEncodingTypes[line[0]].signalRepresentation = signals;
        actIndex++;
    }
